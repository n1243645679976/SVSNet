import torch
from .lib_sincnet import SincNet
...
#        y = [self.ds(_x.reshape(1, 1, -1)) for _x in x]
    ...
    def forward(self, x, label):
        y1 = self.encode_frame_embedding([_x1 for _x1, _x2 in x]) # BTF
        y2 = self.encode_frame_embedding([_x2 for _x1, _x2 in x]) # BTF
...
class CEmodel(model):
    def __init__(self, device='cpu'):
        super(CEmodel, self).__init__(device)
        del self.derive_model
        self.derive_model = torch.nn.Sequential(
                                torch.nn.Linear(256, 128),
                                torch.nn.ReLU(), torch.nn.Dropout(0.3),
                                torch.nn.Linear(128, 2)
                                )
        for name, para in self.derive_model.named_parameters():
            if 'weight' in name:
                torch.nn.init.xavier_uniform_(para, gain=1.414)
                print('init '+name+' by xavier_uniform')
            if 'bias' in name:
    ...
    def forward(self, x, label):
        ...
        y = y.reshape(label.shape[0], 2)
        return self.criterion(y, label), torch.argmax(y, dim=1)
