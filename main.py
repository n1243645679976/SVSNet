...
if __name__ == '__main__':
    ...
    parser.add_argument('--train-list', default='../../feature/sim_net/sim_list_train.txt', help='list for training')
    ...
    parser.add_argument('--exp-dir', default='result/SVSNet/wav', help='result folder, used for saving models and results')
    ...
    if args.model_type == 'regression':
        from model.lib_resnet import model as model_class
    elif args.model_type == 'classification':
        from model.lib_resnet import CEmodel as model_class
    ...
    dataloader = DataLoader(dataset, batch_size=args.batch_size, shuffle=True, collate_fn=cofn, drop_last=True)
    ...
    logger = Logger(exp_dir=args.exp_dir, dataset=dataset)
    if not args.continue_from:
        if os.path.isdir(os.path.join(args.exp_dir, 'models')):
            ...
            if len(models) > 0:
                ...
                print(f'Find previous checkpoint: {args.continue_from}')
    ...
    if args.continue_from:
        ...
        start_epoch = model_['epoch'] + 1
        logger.logger = copy.deepcopy(model_['logger'].logger)
        del model_
    ...
    for i in range(start_epoch, args.epoch):
        if not args.testing:
            ...
            for t, (x, label, z) in enumerate(dataloader):
                ...
                optimizer.step()
                optimizer.zero_grad()
                logger.append(i, t, loss, y, label, z, 'training')
        ...
        for t, (x, label, z) in enumerate(dataloader):
            ...
            logger.append(i, t, loss, y, label, z, 'testing')
        logger.log(i, lr_scheduler.optimizer.param_groups[0]['lr'])
        lr_scheduler.step(logger.logger['loss'][-1])
        logger.reset_log()
        ...
        torch.save({'model':model.state_dict(), 'optimizer':optimizer.state_dict(), 'scheduler':lr_scheduler.state_dict(), 'logger':logger, 'epoch':i},
                    os.path.join(os.path.join(args.exp_dir, f'models/checkpoint.ep.{i:0>5d}.pt').format(i)))
